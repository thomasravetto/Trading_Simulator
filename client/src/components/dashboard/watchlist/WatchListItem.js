...
function WatchListItem (props) {
    const asset_info = props.watchlist.information;
    const asset_prices = props.watchlist.prices;
    ...
    const percentage_from_yesterday = calculate_percentage(
        Number(asset_prices[0].price),
        Number(asset_prices[asset_prices.length - 1].price)
    )
        ...
        <div className='watchlist_item'>
            ...
            <div className='watchlist_prices_graph_container'>
                <div className='watchlist_item_prices'>
                    <h2>{asset_prices[0].price.slice(0, -2)}$</h2>
